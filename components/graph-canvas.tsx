...
export const GraphCanvas = forwardRef<GraphCanvasRef, GraphCanvasProps>(function GraphCanvas(
  ...
  const draw = useCallback(() => {
    ...
    graph.edges.forEach((edge) => {
      ...
      // Check if it's a loop (self-edge)
  ...
  function drawVertex(ctx: CanvasRenderingContext2D, vertex: Vertex, isSelected: boolean, isEdgeSource: boolean) {
    ...
    // Glow effect for selected/source vertices
  ...
  function drawLoop(
    ...
    // Position the loop at the top-right of the vertex
  ...
  function getEdgeAtPosition(x: number, y: number): Edge | null {
    ...
    for (const edge of graph.edges) {
      ...
      if (edge.source === edge.target) {
        ...
        const distToLoopCenter = Math.sqrt(
          (adjustedX - loopCenterX) ** 2 + (adjustedY - loopCenterY) ** 2
        )
        // Check if click is on the loop circle (within tolerance)
  ...
  function handleMouseDown(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    switch (tool) {
      ...
      case "edge":
        if (vertex) {
          if (isCreatingEdge && edgeSourceId) {
            // Permite criar loop (aresta para si mesmo)
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full min-h-[400px] bg-background rounded-lg overflow-hidden border border-border">
      <canvas
        ref={canvasRef}
