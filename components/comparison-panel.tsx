  ...
  return (
    <div className="p-4 space-y-6 overflow-auto max-h-[calc(100vh-200px)]">
      <div className="space-y-3">
        ...
        {otherGraphs.length === 0 && <p className="text-xs text-muted-foreground text-center">Crie outro grafo para poder comparar</p>}
      </div>
          ...
          <div className="space-y-3">
            ...
            <div
              className={`p-3 rounded-md border ${
                isomorphismResult?.isIsomorphic ? "bg-green-500/10 border-green-500/30" : "bg-red-500/10 border-red-500/30"
              }`}
            >
              <div className="flex items-center gap-2">
                {isomorphismResult?.isIsomorphic ? <CheckCircle2 className="h-5 w-5 text-green-500" /> : <XCircle className="h-5 w-5 text-red-500" />}
                <span className="font-medium">{isomorphismResult?.isIsomorphic ? "Os grafos SÃO isomorfos" : "Os grafos NÃO são isomorfos"}</span>
              </div>
              {isomorphismResult?.reason && <p className="text-xs text-muted-foreground mt-1">{isomorphismResult.reason}</p>}
...
function StatusIcon({ value }: { value?: boolean }) {
  ...
  return value ? <CheckCircle2 className="h-3.5 w-3.5 text-green-500 inline" /> : <XCircle className="h-3.5 w-3.5 text-red-500 inline" />;
}
