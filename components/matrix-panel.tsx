    ...
    <div className="p-4">
      <Tabs defaultValue="adjacency">
        <TabsList className="w-full">
          <TabsTrigger value="adjacency" className="flex-1">Adjacência</TabsTrigger>
          <TabsTrigger value="incidence" className="flex-1">Incidência</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="adjacency" className="mt-4 space-y-4">
          ...
          {copiedMatrix === "adj" && (
            <p className="text-xs text-green-500">Copiado!</p>
          )}
            ...
            <table className="text-xs border-collapse">
              <thead>
                <tr>
                  ...
                  {adjMatrix.labels.map((label) => (
                    <th key={label} className="p-1.5 border border-border bg-muted/50 font-medium min-w-[32px]">
                      {label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {adjMatrix.matrix.map((row, i) => (
                  <tr key={i}>
                    <th className="p-1.5 border border-border bg-muted/50 font-medium">
                      {adjMatrix.labels[i]}
                    </th>
                    {row.map((cell, j) => (
                      <td
                        ...
                        className={cn(
                          ...
                          cell > 0 && "bg-primary/20 text-primary font-medium"
                        )}
                      >
                        {cell}
                      </td>
          ...
          <p className="text-xs text-muted-foreground">
            {activeGraph.directed
              ? "Matriz[i][j] = peso da aresta de i para j"
              : "Matriz simétrica: Matriz[i][j] = Matriz[j][i]"}
          </p>
        </TabsContent>
